...
private:
    ...
    struct EncoderStatusBits
    {
        ...
        EncoderStatusBits(uint8_t const value) : _Value(value)
        {
            static_assert(sizeof(*this) == sizeof(uint8_t));
        }
    };
    ...
    struct SecondaryInterruptStatusBits
    {
        ...
        SecondaryInterruptStatusBits(uint8_t const value) : _Value(value)
        {
            static_assert(sizeof(*this) == sizeof(uint8_t));
        }
    };
...
private:
    ...
    {
    public:
        AutoTransmission(TwoWire& wire, uint8_t const address) : m_Wire(wire)
        {
